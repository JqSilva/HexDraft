  ...
  const renderDetailView = () => {
    ...
    return (
      <div className="w-full flex flex-col p-4 md:p-6 animate-in fade-in duration-300">
        ...
        {/* Runas Populares Section */}
        {buildsList && buildsList.length > 0 && (
          <div className="bg-[#0b0b0f] border border-border-warm rounded-sm p-5 tech-corners shadow-2xl mb-6 select-none">
            <h3 className="text-sm text-cyan-400 font-black uppercase tracking-[0.3em] italic mb-5">
              Páginas de Runas Populares
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {buildsList.map((b: any, idx: number) => {
                const runasObj = b.runes;
                if (!runasObj || !runasObj.selections || runasObj.selections.length === 0) return null;

                const primaryTree = RUNE_TREES[runasObj.primaryStyleId];
                const secondaryTree = RUNE_TREES[runasObj.subStyleId];
                const keystoneId = runasObj.selections[0];
                const keystone = hydrateAsset('runes', keystoneId);

                const tabWinrate = b.special_notes?.winrate || (b.is_default ? (champ.meta?.winRate || 50.0) : 49.2);
                const tabGames = b.special_notes?.games || (b.is_default ? Math.round(champ.matches * 0.72) : Math.round(champ.matches * 0.28));

                // Selecciones primarias y secundarias
                const pSelections = runasObj.selections.slice(0, 4);
                const sSelections = runasObj.selections.slice(4, 6);

                const bSkills = b.skills;
                let bSkillOrder = "Q > W > E";
                if (bSkills) {
                  const order = [
                    { key: "Q", pos: bSkills.skillLevelUp1 || 1 },
                    { key: "W", pos: bSkills.skillLevelUp2 || 2 },
                    { key: "E", pos: bSkills.skillLevelUp3 || 3 }
                  ];
                  bSkillOrder = order.sort((a, b) => a.pos - b.pos).map(x => x.key).join(" > ");
                }

                return (
                  <div key={idx} className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 bg-[#0c0c0f]/80 border border-border-warm rounded-sm hover:border-slate-800 transition-all">
                    <div className="flex items-center gap-3">
                      {/* Keystone y subStyle */}
                      <div className="flex -space-x-2 shrink-0">
                        <div className="w-12 h-12 rounded-full bg-black/60 border border-border-warm flex items-center justify-center overflow-hidden z-10" title={keystone?.name}>
                          {keystone?.icon && <img src={keystone.icon} className="w-4/5 h-4/5 object-contain" alt="keystone" />}
                        </div>
                        <div className="w-8 h-8 rounded-full bg-black/65 border border-border-warm/80 flex items-center justify-center overflow-hidden self-end translate-y-1" title={primaryTree?.name}>
                          {primaryTree?.icon && <img src={primaryTree.icon} className="w-3/5 h-3/5 object-contain" alt="primary style" />}
                        </div>
                      </div>

                      {/* Nombres y árbol */}
                      <div>
                        <span className="block text-xs font-black text-white uppercase tracking-wide">
                          {keystone?.name || "Runa Clave"}
                        </span>
                    ...
                    {/* Las runas individuales mostradas linealmente */}
                    <div className="flex flex-wrap gap-1 items-center justify-center max-w-[150px]">
                      {pSelections.slice(1).map((rId: number, rIdx: number) => {
                        const r = hydrateAsset('runes', rId);
                        return (
                          <div key={rIdx} className="w-6 h-6 rounded-full bg-slate-900 border border-slate-800 flex items-center justify-center" title={r?.name}>
                            {r?.icon && <img src={r.icon} className="w-3/4 h-3/4 object-contain" alt="primary selection" />}
                          </div>
                        );
                      })}
                      <span className="text-slate-600 text-xs px-0.5">|</span>
                      {sSelections.map((rId: number, rIdx: number) => {
                        const r = hydrateAsset('runes', rId);
                        return (
                          <div key={rIdx} className="w-6 h-6 rounded-full bg-slate-900 border border-slate-800 flex items-center justify-center" title={r?.name}>
                            {r?.icon && <img src={r.icon} className="w-3/4 h-3/4 object-contain" alt="secondary selection" />}
                          </div>
                        );
                      })}
                    </div>
                    ...
                    {/* Stats */}
                    <div className="flex sm:flex-col items-end gap-2 sm:gap-0 font-mono text-right shrink-0">
                      <span className="text-xs font-black text-emerald-400">
                        {tabWinrate.toFixed(1)}% WR
                      </span>
                    ...
                    {/* Botón para importar directamente */}
                    <button
                      onClick={async () => {
                        try {
                          const name = champ.name;
                          const runePayload = {
                            name: `HexDraft: ${name}`,
                            primaryStyleId: runasObj.primaryStyleId,
                            subStyleId: runasObj.subStyleId,
                            selectedPerkIds: [
                              ...runasObj.selections,
                              ...(runasObj.shards || [])
                            ]
                          ...
                          const spell1 = b.summoners?.[0] || 4;
                          const spell2 = b.summoners?.[1] || 12;
                          ...
                          alert(`¡Playstyle (Runas + Items) de ${name} importados con éxito!`);
                        } catch (err) {
                          console.error(err);
                          alert('Error al intentar conectar con el cliente LCU.');
                        }
                      }}
                      className="px-2.5 py-1.5 bg-purple-accent/15 border border-purple-accent/35 text-purple-accent hover:bg-purple-accent hover:text-white transition-all text-[10px] font-black uppercase tracking-wider rounded-sm cursor-pointer select-none active:scale-95"
                    >
                      ⚡ Importar
                    </button>
                  </div>
