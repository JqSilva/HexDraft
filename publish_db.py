...
if os.path.exists(config_path):
    try:
        with open(config_path, "r", encoding="utf-8") as f:
            ...
            print(f"✅ Configuración cargada desde data/config.json (Modo: {config.get('mode')})")
    except Exception as e:
        print(f"⚠️ Error leyendo data/config.json: {e}")

# Fallbacks
if not github_token:
    github_token = os.environ.get("GITHUB_TOKEN")
    if github_token:
        print("✅ Usando GITHUB_TOKEN desde variable de entorno.")

if not github_repo:
    github_repo = "JqSilva/HexDraft" # Fallback por defecto

if not github_token:
    print("❌ ERROR: No se encontró token de GitHub. Configura 'github_token' en data/config.json o define la variable de entorno GITHUB_TOKEN.")
    sys.exit(1)
...
if os.path.exists(meta_path):
    try:
        with open(meta_path, "r", encoding="utf-8") as f:
            ...
            print(f"✅ Parche detectado en meta-cache.json: {patch}")
    except Exception as e:
        print(f"⚠️ Error leyendo meta-cache.json: {e}")
...
if patch == "-":
    try:
        ...
        if row:
            patch = row[0]
            print(f"✅ Parche detectado en base de datos SQLite: {patch}")
        conn.close()
    except Exception as e:
        print(f"⚠️ No se pudo leer el parche desde la base de datos: {e}")
...
try:
    with urllib.request.urlopen(req) as response:
        ...
        if remote_manifest and "version" in remote_manifest:
            ...
            print(f"📡 Última versión publicada detectada: v{latest_version}")
except urllib.error.HTTPError as e:
    if e.code == 404:
        print("📡 No se encontraron publicaciones previas en GitHub. Iniciando desde v0.")
    else:
        print(f"⚠️ Error consultando API de GitHub ({e.code}). Se asume v0.")
except Exception as e:
    print(f"⚠️ Error consultando versión remota anterior: {e}. Se asume v0.")
...
create_data = json.dumps({
    ...
    "name": f"Base de datos v{new_version} - Parche {patch}",
...
try:
    with urllib.request.urlopen(req_create) as response:
        ...
        print(f"✅ Release creada con éxito. URL: {html_url}")
except urllib.error.HTTPError as e:
...
try:
    ...
    with urllib.request.urlopen(req_upload) as response:
        print("✅ Base de datos SQLite subida con éxito como asset de la release.")
except urllib.error.HTTPError as e:
...
try:
    ...
    print(f"💾 Manifest local guardado en: {db_version_path}")
except Exception as e:
    print(f"⚠️ Error guardando el manifest local: {e}")
